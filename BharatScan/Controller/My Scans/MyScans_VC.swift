...
@available(iOS 14.0, *)
class MyScans_VC: UIViewController,CAAnimationDelegate,UITextFieldDelegate{
    ...
    override func viewWillLayoutSubviews() {
        ...
        self.view_navBar.backgroundColor = THEME_COLOR_NAVIGATION
    }
    ...
    func setUIConfiguration(){
        ...
        panGesture = UIPanGestureRecognizer(target: self, action: #selector(MyScans_VC.draggedView(_:)))
    ...
    @IBAction func downMenuClicked(_ sender: Any) {
        self.go_toCameraScreen()
    }
    
    
    func go_toCameraScreen(){
        let camera_VC = self.storyboard?.instantiateViewController(withIdentifier: "Camera_VC") as! Camera_VC
        camera_VC.modalPresentationStyle = .fullScreen
//        camera_VC.isfromHomeScreen = true
        self.navigationController?.pushViewController(camera_VC, animated: true)
        //        self.present(vc, animated: true, completion: nil)
    }
...
@available(iOS 14.0, *)
extension MyScans_VC : UICollectionViewDataSource,UICollectionViewDelegate,UICollectionViewDelegateFlowLayout{
...
extension MyScans_VC : UITableViewDataSource,UITableViewDelegate{
